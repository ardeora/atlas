...
export default class Atlas {
  ...
  private initializeTiles(): Tile[] {
    ...
    const defaultTiles = tileArray.filter((tile) => tile.default);
  ...
  public showTile(name: string): boolean {
    const selectedTiles = this.tiles.filter((tile) => tile.name == name);
  ...
  public addTile(config: TileOptions): void {
    const { name } = config;
  ...
  public deleteTile(name: string): void {
    const tilesToDelete = this.tiles.filter((tile) => tile.name == name);
